...
class PotentialNullDereferenceInspection : GlobalInspectionTool() {
            ...
            override fun visitFile(file: PsiFile) {
                ...
                if(holder.hasResults()) {
                    globalContext.refManager.getReference(file)?.let{
                        problemDescriptionsProcessor.addProblemElement(it, *holder.results.toTypedArray())
                    }
                }
            }
        })
    ...
    private fun getNullnessOfExpression(expr: PsiExpression?): Nullness = when (expr) {
        ...
        is PsiMethodCallExpression -> expr.resolveMethod()?.let {
            inferReturnNullness(it)
        } ?: Nullness.MAYBE_NULL
            ...
            for (statement in body.statements) {
                ...
                when (statement) {
                    ...
                    is PsiExpressionStatement -> {
                        ...
                        assignment.rExpression?.takeIf { !onlyMethodCalls || it is PsiMethodCallExpression }
                        ?.let { add(it to getNullnessOfExpression(it)) }
                    }
                }
            }
    ...
    private fun inferReturnNullness(method: PsiMethod, visited: MutableSet<PsiMethod> = mutableSetOf()): Nullness {
        ...
        return returnValues.map { returnExpr ->
            when(returnExpr) {
                ...
                is PsiMethodCallExpression -> returnExpr.resolveMethod()?.let {
                    inferReturnNullness(it, visited)} ?:Nullness.MAYBE_NULL
                ...
                is PsiReferenceExpression ->{
                    ...
                    analyzeAssignmentsTo(returnExpr, resolved, enclosingMethod).lastOrNull()?.second ?: Nullness.MAYBE_NULL
                }
                else -> Nullness.MAYBE_NULL
            }
...
enum class Nullness {
    ...
    fun mergeWith(other: Nullness): Nullness = if(this == other) this else MAYBE_NULL
}

// You can define multiple inspections in one .inspection.kts file 
listOf(
    InspectionKts(
        id = "PotentialNullDereferenceInspection", // inspection id (used in qodana.yaml)
        globalTool = PotentialNullDereferenceInspection(),
        name = "Template custom inspection inspections/PotentialNullDereferenceInspection.inspection.kts", // Inspection name, displayed in UI
